...
async def play(_, message: Message):
    ...
    try:
        ...
        await lel.edit(
            f"<i> الحساب المساعد ليس في هذه الدردشة ، اطلب من المسؤول إرسال `/انضم` لأول مرة أو الإضافة @{user.username} يدويا</i>"
        )
        return
    text_links=None
        ...
        for administrator in administrators:
            if administrator == message.from_user.id:
                ...
                try:
                    ...
                    # print(e)
                    await lel.edit(
                        f"<b>🔴 خطأ هناك ضغط علي الحساب المساعد 🔴 \n لا يمكن الانضمام إلى مجموعتك بسبب الطلبات الكثيفة على الحساب المساعد! تأكد من عدم حظر الحساب المساعد @{user.username} في المجموعة."
                        f"\n\nأو أضف الحساب المساعد @{user.username} يدويًا إلى مجموعتك وحاول مرة أخرى</b>",
                    )
    try:
        await USER.get_chat(chid)
        # lmoa = await client.get_chat_member(chid,wew)
        ...
        await lel.edit(
            f"<i>الحساب المساعد ليس في هذه الدردشة ، اطلب من المسؤول الإرسال `/انضم` او اضف البوت يدويا @{user.username}</i>"
        )
        return
