...
@Client.on_message(filters.command(["saavn","سافن تحميل",f"saavn@{BOT_USERNAME}",f"سافن تحميل@{BOT_USERNAME}"]))
async def jssong(_, message):
    global is_downloading
    if len(message.command) < 2:
        await message.reply_text("/سافن تحميل + اسم الاغنية علي موقع سافن.")
        return
    if is_downloading:
        await message.reply_text(
            "هناك تحميل اخر قيد التقدم."
        )
        return
    is_downloading = True
...
@Client.on_message(filters.command(["deezer","ديزر تحميل",f"deezer@{BOT_USERNAME}",f"ديزر تحميل@{BOT_USERNAME}"]))
async def deezsong(_, message):
    global is_downloading
    if len(message.command) < 2:
        await message.reply_text("/ديزر تحميل + اسم الاغنية علي موقع ديزر")
        return
    if is_downloading:
        await message.reply_text(
            "هناك تحميل اخر قيد التقدم."
